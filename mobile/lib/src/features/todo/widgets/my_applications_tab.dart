...
/// 我的申请Tab
class MyApplicationsTab extends HookConsumerWidget {
  const MyApplicationsTab({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(myApplicationsProvider);
    final notifier = ref.read(myApplicationsProvider.notifier);
  ...
  Widget _buildBody(
    ...
    MyApplicationsState state,
    MyApplicationsNotifier notifier,
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildApplicationCard(context, state.items[index]);
      },
    );
  }

  Widget _buildLoadingIndicator(
      BuildContext context, MyApplicationsState state) {
  ...
  Widget _buildApplicationCard(BuildContext context, ApprovalRequest item) {
    return Card(
      child: InkWell(
        onTap: () {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  ApplicationDetailScreen(requestId: item.id),
            ),
          );
        },
