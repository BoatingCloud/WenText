  ...
  const columns: ColumnsType<PhysicalArchive> = [
    ...
    { title: '档案代码', dataIndex: 'archiveCode', key: 'archiveCode', width: 140 },
    {
      ...
      render: (text: string, record: PhysicalArchive) => (
        <a onClick={() => navigate(`/admin/physical-archives/${record.id}`)}>{text}</a>
      ),
    },
    {
      title: '所属公司',
      ...
      render: (_value, record) => (
        <Space>
          <Button
            size="small"
            icon={<EyeOutlined />}
            onClick={() => navigate(`/admin/physical-archives/${record.id}`)}
          >
            查看
          </Button>
          {canUpdate && (
            <Button
              size="small"
              icon={<EditOutlined />}
              onClick={() => navigate(`/admin/physical-archives/${record.id}/edit`)}
            >
              编辑
            </Button>
          )}
            ...
            <Space wrap>
              ...
              {canCreate && (
                <Button
                  type="primary"
                  icon={<PlusOutlined />}
                  onClick={() => navigate('/admin/physical-archives/create')}
                >
                  新增档案
                </Button>
              )}
