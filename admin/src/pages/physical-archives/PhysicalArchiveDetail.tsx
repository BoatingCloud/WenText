...
type BorrowRequestFormValues = {
  borrowReason?: string;
  expectedBorrowAt?: dayjs.Dayjs;
  expectedReturnAt?: dayjs.Dayjs;
};
...
const PhysicalArchiveDetail: React.FC = () => {
  ...
  const [borrowRequestModalOpen, setBorrowRequestModalOpen] = useState(false);
  const [borrowRequestForm] = Form.useForm<BorrowRequestFormValues>();
  ...
  const borrowMode: ArchiveBorrowMode = (settingsData?.data?.data as any)?.archiveBorrowMode || 'direct';
  ...
  const borrowRequestMutation = useMutation({
    mutationFn: (values: BorrowRequestFormValues) =>
      borrowRequestApi.create({
        archiveId: id!,
        borrowReason: values.borrowReason,
        expectedBorrowAt: values.expectedBorrowAt?.toISOString(),
  ...
  if (!archive) {
    return (
      <Card>
        ...
        <Button type="link" onClick={() => navigate('/physical-archives')}>返回列表</Button>
      </Card>
    );
  }
  ...
  return (
    <>
      <Card
        title={
          <Space>
            <Button icon={<ArrowLeftOutlined />} type="text" onClick={() => navigate('/physical-archives')} />
            <span>档案详情</span>
          ...
          <Space wrap>
            ...
            {canBorrow && st === 'IN_STOCK' && (ws === 'ARCHIVED' || ws === 'RETURNED') && borrowMode === 'workflow' && (
              <Button type="primary" onClick={() => {
                borrowRequestForm.resetFields();
                setBorrowRequestModalOpen(true);
              }}>申请借阅</Button>
            )}
            {canReturn && st === 'BORROWED' && (
              <Button onClick={() => {
                returnForm.setFieldsValue({ returnedAt: dayjs(), returnRemark: '' });
            ...
            {canUpdate && (
              <Button type="primary" icon={<EditOutlined />} onClick={() => navigate(`/physical-archives/${id}/edit`)}>
                编辑
              </Button>
            )}
          </Space>
      ...
      {/* 申请借阅 Modal (工作流模式) */}
      <Modal
        title={`申请借阅 - ${archive.archiveNo}`}
        open={borrowRequestModalOpen}
        onCancel={() => { setBorrowRequestModalOpen(false); borrowRequestForm.resetFields(); }}
        onOk={async () => {
          const values = await borrowRequestForm.validateFields();
          await borrowRequestMutation.mutateAsync(values);
          message.success('借阅申请已提交，请等待审批');
          setBorrowRequestModalOpen(false);
          borrowRequestForm.resetFields();
        }}
        confirmLoading={borrowRequestMutation.isPending}
      >
        <Form form={borrowRequestForm} layout="vertical">
          <Form.Item label="借阅理由" name="borrowReason" rules={[{ required: true, message: '请输入借阅理由' }]}>
            <Input.TextArea rows={3} placeholder="请输入借阅理由" />
          </Form.Item>
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item label="预计借阅日期" name="expectedBorrowAt">
                <DatePicker style={{ width: '100%' }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item label="预计归还日期" name="expectedReturnAt">
                <DatePicker style={{ width: '100%' }} />
              </Form.Item>
            </Col>
          </Row>
