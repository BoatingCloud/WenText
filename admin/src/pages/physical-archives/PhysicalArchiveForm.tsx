  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit && editing) {
        ...
        navigate(`/physical-archives/${editing.id}`);
        return;
      }
      ...
      const preserve = pickFields(values, [
        ...
        'ownerDepartment', 'accessLevel', 'workflowStatus',
      ]);
        ...
        <Col span={8}>
          <Form.Item label="所属公司" name="companyCode">
            <Select
              allowClear
              placeholder="选择所属公司"
              options={companyCatalog.map((c) => ({ label: c.name, value: c.code }))}
            />
          </Form.Item>
        </Col>
      </Row>
